...
export class SocialUserService {
  ...
  async findByProvider(
    ...
    const user = await this.prisma.socialUser.findUnique({ where: { provider, providerAccountId } });
   ...
   */
  async createSocialUser(dto: CreateSocialUserDto): Promise<any> {
    const { userIdx, provider, providerAccountId } = dto;

    const exists = await this.findByProvider(provider, providerAccountId);
   ...
   */
  async deleteSocialUser(dto: DeleteSocialUserDto): Promise<any> {
    const { provider, providerAccountId } = dto;

    const socialUser = await this.findByProvider(provider, providerAccountId);
    if (!socialUser) throw new NotFoundException('소셜 유저를 찾을 수 없습니다.');

    const deletedSocialUser = await this.prisma.socialUser.delete({
